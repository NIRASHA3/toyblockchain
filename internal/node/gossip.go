...
// from the origin peer only when that origin is a configured peer.
func (n *Node) ReceivePeerBlock(ctx context.Context, block blockchain.Block, origin string) (BlockGossipResult, error) {
	...
	if err != nil {
		if errors.Is(err, ErrBlockDoesNotExtendHead) && origin != "" {
			if !n.IsKnownPeer(origin) {
				return result, fmt.Errorf("%w; fork resolution skipped because origin %s is not a configured peer", err, origin)
			}
